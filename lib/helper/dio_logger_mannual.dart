...
class PrettyDioLogger extends Interceptor {
  ...
  void onRequest(RequestOptions options, RequestInterceptorHandler handler) {
    ...
    if (requestBody && options.method != 'GET') {
      ...
      if (data != null) {
        if (data is Map) {
          _printMapAsTable(options.data as Map?, header: 'Sending Body');
  ...
  void onError(DioException err, ErrorInterceptorHandler handler) {
    if (error) {
      if (err.type == DioException.badResponse) {
  ...
  void _printResponse(Response response) {
    if (response.data != null) {
      if(responseBobyInJson){
        var logger = Logger();
        dynamic parsedJson = json.decode(response.data);
        if (parsedJson is Map<String, dynamic>) {
          logger.d(parsedJson);
        } else if (parsedJson is List<dynamic>){
          logger.d(parsedJson);
        } else {
          _printBlock(parsedJson);
        }
      } else{
